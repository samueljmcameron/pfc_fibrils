...
// maximum round off error by subtracting E(x+a) from E(x) is at most //
// E(x+a)-E(x) = C*(fabs(E(x+a))+fabs(E(x))) ~ 2*C*fabs(E(x+a)) (for  //
// small a). Therefore, since I'm using the usual  central difference //
// approximation for derivatives, the largest error from round off in //
// this calculation will be ~2*C*fabs(E)/(2*dx). That means that the  //
// size of the convergence criteria on gradient descent, conv, must   //
// be larger than this round off error, conv>2*C*fabs(E)/(2*dx). To   //
// be safe, I've made dx = C*fabs(E)/(0.5*conv), which ensures the    //
// error is half the size of the convergence criterion. Note that I'm //
// not considering truncation error (from finite-difference's Taylor  //
...
// requires re-evaluating the 
{
  ...
  for (i = 1; i <= x_size; i++) {
    ...
    E_p[i] = E_calc(p,x_cp,c,s,y,r,rf_fib,y_cp,r_cp,conv,itmax,
		    mpt,last_mpt,ns,max_size); // compute E for this small step dx
    
    
    x_cp[i] -= 2*dx; // remove small step, and then subtract another
    
    E_m[i] = E_calc(p,x_cp,c,s,y,r,rf_fib,y_cp,r_cp,conv,itmax,
		    mpt,last_mpt,ns,max_size); // compute E for this small step -dx
