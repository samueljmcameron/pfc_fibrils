...
if __name__=="__main__":
    ...
    Lambdas = np.linspace(0,40,num=41,endpoint=True)
    ...
    while (i < len(Lambdas)):
        ...
        Ei,Ri,etai,deltai,surftwisti,modulusi = run.get_all_observables('observables',str2float=True)
        ...
        if (Ei > 0.1*FAILED_E):
            ...
            for j,Lambda in enumerate(Lambdas[i:]):
                ...
                rp = ReadParams(scan=scan,loadsuf=loadsuf,savesuf=savesuf)
                run = SingleRun(rp,scan_dir=scan_dir)
                run.write_observables(Ei,Ri,etai,deltai,surftwisti,"\\Lambda",
                                      modulus=modulusi)
