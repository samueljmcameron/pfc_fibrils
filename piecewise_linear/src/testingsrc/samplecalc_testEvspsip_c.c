...
int main(int argc, char **argv)
{
  ...
  initialize_file(&Evspsip_c,argv[1],"Evspsip_c",p);

  double psip_c0 = 0.5;
  double psip_cf = 5.0;
  int num = 1000;

  double dpsip_c = (psip_cf-psip_c0)/num;
  
  int i;

  for (i = 1; i <= 1000; i++) {

    p.psip_c = dpsip_c*i+psip_c0;

    fprintf(Evspsip_c,"%13.6e\t%13.6e\t%13.6e\n",p.psip_c,Efunc(&p),dEdpsip_c(&p));
