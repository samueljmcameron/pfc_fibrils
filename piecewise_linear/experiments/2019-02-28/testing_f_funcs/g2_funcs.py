...
if __name__ == "__main__":
    ...
    run = SingleRun(rp,executable="../../../bin/g2_func")
    # run C executable.
    run.run_exe()
    dataname = run.mv_file('g2_func')
    ...
    axarr[0].plot(zetas,g2s,'.',color=colors[0])
    axarr[0].set_ylabel(r"$f_1$")
    axarr[1].plot(zetas,dg2dxis,'.',color=colors[1])
    axarr[1].set_ylabel(r"$\partial f_1/\partial \xi$")
    axarr[2].plot(zetas,dg2dzetas,'.',color=colors[2])
