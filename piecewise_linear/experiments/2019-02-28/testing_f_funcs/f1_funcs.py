...
if __name__ == "__main__":
    ...
    run = SingleRun(rp,executable="../../../bin/f1_func")
    # run C executable.
    run.run_exe()
    dataname = run.mv_file('f1_func')
    ...
    axarr[0].plot(zetas,f1s,'.',color=colors[0])
    axarr[0].set_ylabel(r"$f_1$")
    axarr[1].plot(zetas,df1dxis,'.',color=colors[1])
    axarr[1].set_ylabel(r"$\partial f_1/\partial \xi$")
    axarr[2].plot(zetas,df1dzetas,'.',color=colors[2])
